...
class _GameDataStore extends DataStore<GameDataStorePayload>
{
	...
	private loadBlackCard()
	{
		...
		return AbcPlatform.getBlackCard(blackCard)
	...
	{
		const data = await AbcPlatform.getWhiteCards(cardIds);
	...
	public hydrate(gameId: string)
	{
		...
		return AbcPlatform.getGame(gameId)
			.then(data =>
			{
				this.update({
					loaded: true,
	...
	public playWhiteCards(cardIds: CardId[] | undefined, userGuid: string)
	{
		...
		return AbcPlatform.playCards(this.state.game.id, userGuid, cardIds)
	...
	public chooseWinner(chooserGuid: string, winningPlayerGuid: string)
	{
		...
		return AbcPlatform.selectWinnerCard(this.state.game.id, chooserGuid, winningPlayerGuid)
			.catch(e => console.error(e));
	}
	...
	public revealNext(userGuid: string)
	{
		...
		return AbcPlatform.revealNext(this.state.game.id, userGuid)
			.catch(e => console.error(e));
	}
	...
	public skipBlack(userGuid: string)
	{
		...
		return AbcPlatform.skipBlack(this.state.game.id, userGuid)
			.catch(e => console.error(e));
	}
	...
	public startRound(userGuid: string)
	{
		...
		return AbcPlatform.startRound(this.state.game.id, userGuid)
			.catch(e => console.error(e));
	}
	...
	public addRandomPlayer(userGuid: string)
	{
		...
		return AbcPlatform.addRandomPlayer(this.state.game.id, userGuid)
			.catch(e => console.error(e));
	}
	...
	public restart(playerGuid: string)
	{
		...
		return AbcPlatform.restart(game.id, playerGuid).then(() =>
		{
			this.update({
				loaded: true
			});
	...
	public forfeit(playerGuid: string, cardsNeeded: number)
	{
		...
		return this.playWhiteCards(toPlay, playerGuid)
			.then(() =>
			{
				AbcPlatform.forfeit(game.id, playerGuid, toPlay);
			});
	}
}
