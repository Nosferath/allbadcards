...
import {BaseGameListManager} from "./BaseGameListManager";
import {IBaseGame} from "./BaseContracts";

const cache = apicache.middleware;

export const RegisterBaseGameEndpoints = <T extends IBaseGame>(
	app: Express,
	clientFolder: string,
	listManager: BaseGameListManager<T>) =>
{
	app.get("/api/abc/games/public", cache("15 seconds"), async (req, res) =>
	{
		...
		try
		{
			const games = await listManager.getGames(parseInt(req.query.zeroBasedPage));
